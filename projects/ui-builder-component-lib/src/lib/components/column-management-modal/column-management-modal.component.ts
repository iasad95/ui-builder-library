...
@Component({
  selector: 'lib-column-management-modal',
...
export class ColumnManagementModalComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<ColumnManagementModalComponent>,
    @Inject(MAT_DIALOG_DATA) public data: ColumnManagementData,
  ) {
  ...
  public handleClickSave(): void {
    this.dialogRef.close({
      ...
      columns: this.originalRightColumns,
      filteredColumns: this.rightColumns,
    });
  }
  public handleSearchStringChange(query: string) {
    if (query.length > 0) {
      ...
      this.originalLeftColumnGroups.forEach((columnGroup) => {
        const filteredColumnGroup: GridColumnGroup = {
          groupName: columnGroup.groupName,
          columns: [],
        };
